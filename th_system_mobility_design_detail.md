...
| `esp32_bridge` | ESP32との通信(micro-ROS) | C++(ESP32側) |
...
- micro-ROSを採用し、ESP32をROS2ノードとして直接統合する。
- トランスポートはシリアル(USB)を第一候補とする。配電盤近傍は電磁ノイズの懸念があるため、Wi-Fi(UDP)接続は現場検証の上で判断する。
- メッセージ: `/esp32/wheel_cmd`(left_speed, right_speed [m/s])、`/esp32/wheel_feedback`(実速度+タイムスタンプ)。
- ROS2側で稼働する `micro-ROS Agent` は、ESP32の予期しない再起動によるセッション切断・再接続(6.2節)を前提に、Agentプロセス自体は再起動せずにセッションの再確立を受け付け続ける設定で常駐させる(Dockerコンテナ内でAgentを永続プロセスとして起動し、個々のセッション切断ではコンテナ・プロセスを再起動しない)。

### 6.2 ESP32側ファームウェア設計

- **ウォッチドッグタイマー**: `wheel_cmd` を一定時間(例: 300ms)受信できない場合、強制的に速度ゼロにする。ROS2側クラッシュ・通信断絶時の必須フェイルセーフ。
- **自動再接続シーケンス**: ESP32がウォッチドッグリセットやバッテリー電圧降下等で予期せず再起動した場合、micro-ROSのXRCE-DDSセッションは切断される。再起動後に手動介入(USB再接続やコンテナ再起動)なしで通信を復旧できるよう、micro-ROSの標準的な再接続パターン(`rmw_uros_ping_agent`によるエージェント生存確認を周期的に行い、未接続時はpublisher/subscriber等のエンティティを再生成して再接続する状態機械: 未接続→エージェント探索→接続→正常動作、のループ)をファームウェアに組み込む。これはmicro-ROSの公式サンプルでも推奨されている標準パターンであり、独自実装はしない。
...
| ESP32⇔ROS2間通信断絶 | ESP32側ウォッチドッグでモーター強制停止(ハードウェア層、即時)。並行して`safety_monitor`が`/esp32/wheel_feedback`の途絶(例: 500ms、ESP32側ウォッチドッグの300msよりやや長めに設定しジッタによる誤検知を避ける)を検知し、`/safety/fault`で`twist_mux`を即座にロックしつつ`mode_manager`に通知、現在のモードによらず`IDLE`へ遷移させる |
| ESP32再接続後 | micro-ROSの自動再接続シーケンス(6.2節)により通信は自動復旧し、`safety_monitor`の`/safety/fault`も解除される(twist_muxのロックも解ける)が、`mode_manager`は`IDLE`に留まる。ハードウェアリセットは想定外の事象であるため、他の異常復帰と同様に明示操作なしでは`FOLLOWING`等へ自動復帰させない |
...
- Docker構成: ROS2 Humbleコンテナ内で `mode_manager` 等の上位ノード群および `rosbridge_suite` を実行し、ESP32とはUSBシリアル経由でmicro-ROS接続する(ホストのUSBデバイスをコンテナにパススルー)。
- ESP32が予期せず再起動した場合、USBデバイスが一瞬切断・再列挙され、デバイスパス(例: `/dev/ttyUSB0`)が変わる可能性がある。固定パスでのパススルー設定だと再接続に失敗しうるため、vendor/product IDベースのudevルール等で安定したデバイスパスを割り当てる構成を推奨する(6.1/6.2節の自動再接続シーケンスと合わせて検証が必要、TBD)。
...
- ESP32とROS2間のトランスポート(シリアル vs Wi-Fi)の最終決定(現場の電磁ノイズ環境次第)
...
- micro-ROS Agentの再接続設定の詳細(シリアル接続でのセッション再確立の挙動確認、再接続にかかる時間の実測)
- ESP32再起動時のUSBデバイス再列挙に対するDockerパススルー設定の堅牢化(8章、udevルールでの固定化の実装・検証)
