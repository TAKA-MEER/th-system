...
// 2026-09-08: S-21 は AT_HOME モード（Spec-modes.md §2.3）から導出する。
// 以前の onsiteTestOpen（S-01 のサブ画面として開くローカルフラグ）は廃止した。
test('AT_HOME なら S-21（当日の入口。mode から導出する）', () => {
  assert.equal(resolveScreen({ ...base, mode: 'AT_HOME' }), 'S21')
})

test('AT_HOME は settingsOpen より優先される（動作系モードが最優先）', () => {
  assert.equal(resolveScreen({ ...base, mode: 'AT_HOME', settingsOpen: true }), 'S21')
})

test('IDLE + settingsOpen なら S-50（AT_HOME を足しても変わらない）', () => {
  assert.equal(resolveScreen({ ...base, mode: 'IDLE', settingsOpen: true }), 'S50')
})

// 廃止したフラグを渡しても無視されること（呼び出し側の消し忘れを黙って拾わない）。
