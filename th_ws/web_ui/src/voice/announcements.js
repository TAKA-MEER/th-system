...
const SAFETY_ANNOUNCEMENTS = [
  entry('A1', S, 'LIDAR_LOST', '停止。LiDAR 異常', 2.01,
    { repeatSec: 10, auto: AUTO.WIRED }),
  entry('A2', S, 'ESP32_DISCONNECTED', '停止。駆動系の通信に異常', 2.34,
    { repeatSec: 10, auto: AUTO.WIRED }),
...
const DEMO_ANNOUNCEMENTS = [
  ...
  entry('N4',  D, '/person/status 初回捕捉', '捕捉。前方 3 メートル', 2.67,
    { auto: AUTO.WIRED }),
  ...
  entry('N15', D, 'TRACKING → PREPARE', '接近 3 メートル。退避準備', 3.17,
    { auto: AUTO.WIRED }),
  ...
  entry('N19', D, 'PREPARE → EVADING', '2 メートル。退避開始', 2.12,
    { auto: AUTO.WIRED }),
  entry('N20', D, '退避走行中', '退避中。秒速 15 センチ', 2.74,
  ...
  entry('N26', D, '予測上限超過 → 捜索旋回', '旋回して捜索', 1.34,
    { auto: AUTO.WIRED }),
  entry('N27', D, '捜索中の再捕捉', '再発見。追従再開', 2.13,
    { auto: AUTO.WIRED }),
  ...
  entry('N35', D, 'SUMMONING 離脱でキャンセル', '呼び寄せ中止', 1.02,
    { auto: AUTO.WIRED }),
  entry('N36', D, '配電盤への移動開始', '配電盤 1 へ移動', 2.26,
    { auto: AUTO.WIRED }),
