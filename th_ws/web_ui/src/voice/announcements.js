...
const SAFETY_ANNOUNCEMENTS = [
  ...
  entry('A2', S, 'ESP32_DISCONNECTED', '停止。駆動系の通信に異常', 1.2,
    { repeatSec: 10, auto: AUTO.WIRED }),
  entry('A3', S, 'E-Stop 発動 (hw / tablet)', '緊急停止', 0.7,
    { auto: AUTO.WIRED }),
  entry('B1', S, 'PERSON_TRACKER_LOST → IDLE 強制', '対象ロスト。待機へ', 1.0,
    { repeatSec: 15, auto: AUTO.WIRED }),
  entry('B2', S, 'タブレットとの通信断 (全モード)', '通信途絶', 0.8,
    { auto: AUTO.WIRED }),
...
const DEMO_ANNOUNCEMENTS = [
  ...
  entry('N8',  D, '加速レート制限が作用', '最大加速度', 0.9,
  ...
  entry('N21', D, '距離回復 → 追従復帰', '距離確保。追従へ', 1.3,
    { auto: AUTO.WIRED }),
  entry('N23', D, 'lost_reason=DETECTION_LOST', '対象ロスト', 0.5,
    { auto: AUTO.WIRED }),
