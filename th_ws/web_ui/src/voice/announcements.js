...
const SAFETY_ANNOUNCEMENTS = [
  ...
  entry('C3', S, '捜索打ち切り (search_timeout_sec)', '発見できず。待機', 1.4,
    { repeatSec: 15,
      note: '/person/search_mode の3状態化が前提 (現状 Bool で打ち切りを区別できない)' }),
  entry('C4', S, '退避不可 (retreat_blocked)', '逃げ場なし。離れて', 1.3,
    { repeatSec: 5,
      note: 'follow_planner の PlannerOutput.retreat_blocked の publish が前提' }),
  entry('C7', S, '進路上障害物で停止 (mapless)', '障害物。停止', 1.0,
    { repeatSec: 10,
      note: 'follow_planner_mapless の MaplessOutput.reason の publish が前提' }),
...
const DEMO_ANNOUNCEMENTS = [
  ...
  entry('N25', D, '予測追従開始', '位置を推定中', 1.0,
    { note: '/person/search_mode の3状態化が前提' }),
  entry('N26', D, '予測上限超過 → 捜索旋回', '旋回して捜索', 1.1,
    { note: '/person/search_mode の3状態化が前提' }),
  entry('N27', D, '捜索中の再捕捉', '再発見。追従再開', 1.4,
    { note: '/person/search_mode の3状態化が前提 (N4 の初回捕捉と区別するため)' }),
  ...
  entry('N32', D, 'Nav2 ゴール受理', '移動開始', 0.8,
    { note: 'summon_navigator が Nav2 の受理を publish していない' }),
  entry('N33', D, '呼び寄せ到着', '到着', 0.4,
    { note: 'SUMMONING→IDLE は到着 (N33) と中止 (N35) の両方で起きるため区別できない' }),
