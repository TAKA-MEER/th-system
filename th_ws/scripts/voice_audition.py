...
docs/voice-and-audience.md §2.5 の「話者 ID を差し替えて一括再生成できる構造」の先行実装。
...
# ── 聞き比べ用の代表フレーズ ──────────────────────────────
# docs/voice-and-audience.md §2.7 から、性格の異なるものを選んである。
#   ・英字を含む (読みが崩れないか)
#   ・最短 (0.7s。潰れないか)
#   ・実運用で最も出やすい安全通知
#   ・復帰系 (安心感が出るか)
...
# 地図あり追従 (FOLLOWING) 専用で、そのモードは Spec-transit.md §7 で優先度低とされている。
# (ID, テキスト, 選定理由)
...
# 機器アナウンスらしく寄せる (docs/voice-and-audience.md §2.5)。
# 話速はやや速め、抑揚は抑え気味。
SYNTH_TUNING = {
    'speedScale':     1.15,
    'intonationScale': 0.8,
