...
// PID 出力(PWM)のスルーレート上限 [PWM/s]。目標速度側のランプ
// (TARGET_RAMP_ACCEL_MPS2、下記)とは別に、PID計算後の出力そのものの変化率を
// 制限する。実機計測(2026-09-11)で、目標速度側のランプだけでは KP/KFF 由来の
// オーバーシュートが実出力の急変に直結すると確認された(特に停止側: 0.55m/s
// からの停止で規定 1.5 m/s^2 の約2倍の実加速度)。原因は「進行方向と逆向きの
// 出力を禁止する非対称クランプ」(pid.h)により、停止側が1制御周期(100ms)で
// ゼロへ落ちていたこと(≈1540 PWM/s相当。0.55m/s定常出力 kff*0.55≈154 PWM が
// 100ms でゼロになる計算から逆算)。
//
// 500 PWM/s で初回検証したが、速度換算の実効加速度は理論上 out_ramp_rate/KFF
// にほぼ一定になる(定常出力 KFF*v へ到達する時間が t=KFF*v/R、その間の速度
// 変化がほぼ v なので平均加速度 ≈ v/t = R/KFF)。500/280≈1.79 m/s^2 で、実測
// (低速は改善したが中高速は 1.9〜2.3 m/s^2 のまま)と一致した。規定 1.5 m/s^2
// に寄せるため 300 PWM/s（300/280≈1.07 m/s^2 相当）へ引き下げる
// (2026-09-11 実機再検証)。
//
// ⚠ 安全上のトレードオフ: obstacle_limiter の STOP や safety_monitor の
// fault_lock は ESP32 からは通常の wheel_cmd=0 として届き、この出力
// スルーレートを経由する(E-Stop/ウォッチドッグの即ゼロ経路とは別)。つまり
// 障害物ブレーキ・fault_lock の実際の停止距離もこの値の分だけ伸びる
// (300 PWM/s では 0.55m/s からの停止に ffTerm 分(154PWM)で約0.51秒、従来の
// 1制御周期(100ms)比で追加 +20〜25cm程度の見込み。実機で要再検証)。
// 物理 E-Stop・ウォッチドッグは Motor::stopAll() を直接呼ぶため無関係
// (このスルーレートをバイパスし従来どおり即時停止する)。
// 詳細: VISION.md「2026-09-11 — 手動ジョグの発進・停止でモーター出力が急変する」
#define PID_OUT_RAMP_RATE  300.0f   // PWM/s
...
// ⚠️ ベンチ試験用の一時バイパス（現在【無効】） ⚠️
// 物理E-Stopスイッチ/外部プルアップ抵抗が未配線だとフローティングで常時
// E-Stop 発動状態になり駆動系試験ができない。その回避用。
// 有効にすると物理非常停止がファーム上でバイパスされる。
// 2026-08-31 に無効化した(DEBT-1 / WP-ESP32-01 / O-5)。
// **再度有効にしてよいのは、物理E-Stopスイッチが未配線のベンチ上だけ。**
// 機体を走らせる前に必ずコメントアウトへ戻すこと。
// 有効・無効は焼いた後 /safety/firmware_flags で確認できる
// (0=無効 / 1=バイパス有効 / 0xFF=旧ファームで不明)。
