...
// (2026-09-11 実機再検証)。
//
// setpoint(目標速度)が非ゼロ間で変化する場合(発進・速度変更・正逆転)にのみ
// 効く。setpoint==0(停止指令)は pid.h の compute() で常に即時0を返し、この
// スルーレートを経由しない。obstacle_limiter の STOP・safety_monitor の
// fault_lock・stale 由来のゼロ出力もすべて setpoint=0 として届くため、
// 一時この経路までスルーレート化していたところ brake_accel_mps2=1.18 の
// 実測前提(PWM即時カット後のドラッグのみでの減速)が崩れ、実機でダンボールに
// 軽く接触する事故が発生した(2026-09-14)。即時0に戻して修正した。
// 詳細: VISION.md「2026-09-11 — 手動ジョグの発進・停止でモーター出力が急変する」
// および「障害物ブレーキが本来より遅く制動していた回帰」
