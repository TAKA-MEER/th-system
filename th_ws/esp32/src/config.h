...
// ⚠️ ベンチ試験用の一時バイパス（既定で無効） ⚠️
// 物理E-Stopスイッチ/外部プルアップ抵抗が未配線だとフローティングで常時
// E-Stop 発動状態になり駆動系試験ができない場合にだけ使う一時措置。
// 物理E-Stopスイッチを配線済みの機体では有効化しないこと(DEBT-1)。
// 有効にした場合は ESTOP_HW フレームの flags bit0(bypass_active) に
// 乗って PC 側 (/safety/firmware_flags) から検出できる。
// 台上試験でどうしても要るときだけ、このコメントを外して有効化する。
// ベンチ試験以外での使用禁止。
