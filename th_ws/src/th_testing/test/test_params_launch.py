...
def test_bringup_declares_enable_route_slam_arg():
    ...
    # 16a: map_and_localization_slam_toolbox_node（WS-9N: 地図凍結+自己位置推定）が
    # enable_route_slam ゲートで起動すること。
    after_exec = src.split("executable='map_and_localization_slam_toolbox_node'", 1)
    assert len(after_exec) == 2, (
        "bringup.launch.py が map_and_localization_slam_toolbox_node を使っていない（WS-9N）")
    assert "enable_route_slam" in after_exec[1].split("))", 1)[0], (
        "map_and_localization_slam_toolbox_node の condition が enable_route_slam を見ていない（WS-8B）")
