...
# th_params パッケージのルートを追加（WP-PARAM-01。同上）
...
# ============================================================
# docs/ ディレクトリ全般の場所解決（WP-PARAM-01）
# ============================================================
#
# resolve_spec_dir() は Spec-modes.md 専用に書かれているため書き換えない
# （WP-STATE-01 時点で通っているテストの前提を壊さないため）。
# WP-PARAM-01 は `docs/plan/detailed/DetailedDesign-names.md` を読む必要があるので、
# 同じ 3 段の解決順序（① 環境変数 TH_REPO_ROOT → ② コンテナ内 /docs →
# ③ __file__ から上に辿る）をそのまま流用した汎用版をここに足す。


class DocsRootNotFoundError(RuntimeError):
    pass


def resolve_docs_root() -> pathlib.Path:
    """`docs/` ディレクトリへの Path を解決する（resolve_spec_dir() と同じ 3 段）。"""
    # ① 環境変数 TH_REPO_ROOT
    env_root = os.environ.get('TH_REPO_ROOT')
    if env_root:
        candidate = pathlib.Path(env_root) / 'docs'
        if candidate.is_dir():
            return candidate
        raise DocsRootNotFoundError(
            f"TH_REPO_ROOT={env_root} が指定されているが {candidate} が見つからない")

    # ② コンテナ内マウント /docs（docker-compose.yml の ../docs:/docs:ro）
    container_candidate = pathlib.Path('/docs')
    ...
    raise DocsRootNotFoundError(
        "docs/ が見つからない。TH_REPO_ROOT を設定するか、コンテナ内で /docs が"
        "マウントされていることを確認してください。")


def detailed_design_path(filename: str) -> pathlib.Path:
    """`docs/plan/detailed/<filename>` への Path。見つからなければ例外を投げる。"""
    path = resolve_docs_root() / 'plan' / 'detailed' / filename
    if not path.is_file():
        raise DocsRootNotFoundError(f"{path} が見つからない")
    return path


@pytest.fixture(scope='session')
def names_md_file() -> pathlib.Path:
    """正本 DetailedDesign-names.md への Path。見つからなければ即 fail する。"""
    try:
        return detailed_design_path('DetailedDesign-names.md')
    except DocsRootNotFoundError as e:
        pytest.fail(str(e))
...
# ============================================================
# th_params の config 読み込み（WP-PARAM-01）
# ============================================================
def th_params_config_dir() -> pathlib.Path:
    """th_params/config への Path。`import th_params` の場所から相対に探す
    （--symlink-install なら site-packages のシンボリックリンク先がソースツリーを指すため、
    colcon build 後・pytest 直接実行のどちらでも解決できる）。"""
    import th_params
    pkg_dir = pathlib.Path(th_params.__file__).resolve().parent
    config_dir = pkg_dir.parent / 'config'
    if not (config_dir / 'registry.yaml').is_file():
        raise FileNotFoundError(f"th_params/config/registry.yaml が見つからない: {config_dir}")
    return config_dir


@pytest.fixture(scope='session')
def registry_path() -> pathlib.Path:
    return th_params_config_dir() / 'registry.yaml'


@pytest.fixture(scope='session')
def registry_rows(registry_path):
    """registry.yaml の最上位（dict のリスト）を返す。"""
    import yaml
