...
理論下限 1.10m・実用 1.3〜1.4m より狭い通路は通せない**（機体を実寸より小さいと嘘をつくと
壁接触リスクになる）。0.75m や 1.1m クラスの狭通路は Nav2 自動計画の対象外とし、手動ジョグ
または教示・再生（記録軌跡をなぞるだけでコストマップの衝突ゲートを通らない）で対応する。

**機能制約（実寸正方形 footprint 前提。数値は実機で要確認）**:

- 直進通路: 自動走行の理論下限 1.10m、実用は 1.3〜1.4m（壁が costmap 上で厚く出る・自己位置誤差・
  追従誤差・経路が帯の中心からずれるぶん）
