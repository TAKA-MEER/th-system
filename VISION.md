...
- ESP32 の独立ウォッチドッグ（300ms）が ROS2 クラッシュ時の最終保証。300ms は「WHEEL_CMD を
  受信できているか」を測る値であり、上流の `/cmd_vel` 発行間隔そのものを測る値ではない。
  esp32_bridge は最新の `/cmd_vel` を一定周期（20Hz）で ESP32 へ再送し続けるため、上流の
  publish 間隔に平常時ジッタ（`docs/network.md` 記載: 0.5〜1.2 秒）があっても、ESP32-PC 間の
  WebSocket リンク自体が生きていれば誤発動しない。ROS2/esp32_bridge プロセスが本当に落ちれば
  再送タイマーごと止まるため、300ms 以内に停止する最終保証は変わらない（2026-08-05 決定・
  実機ログで誤発動を確認し対策）。
