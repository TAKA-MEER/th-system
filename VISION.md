...
| FOLLOWING | 2 | 地図あり追従（Nav2 経由）。広い試験場での見失い防止に使える可能性があるが、**実装・調整の優先度は低い**（§6） |
...
| 7 | AT_PANEL の作業フロー | ❓ 要確認 | 外部システム連携が未確定（§6） |
| 8 | 試験場での SLAM 走行（実機） | ❓ 要確認 | 地図作成・保存・再利用の実機運用手順を要確認 |
| 9 | FOLLOWING（地図あり追従） | ⏸ 優先度低 | §6 参照 |
| 10 | 待機中の指定位置への移動 | ⏸ 未着手 | §6 参照。位置指定は呼び寄せ / 地図タップ / 専用指定デバイスの 3 方式を想定（暫定）。地図タップの受け口（/manual/target_pose）のみ実装済み |
