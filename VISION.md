...
- **話者**: オリジナルキャラクター構想との統合を前提に、ある程度オリジナリティのある少女の声を目指す。
  VOICEVOX を第一候補とする（過去に使用実績あり）。認知度の高い話者は聞いた瞬間に既存キャラと
  判別されオリジナリティを損なうため、認知度の低い話者から選定する。話速はやや速め、抑揚は
  抑え気味にして機器アナウンスらしい印象に寄せる
...
| A2 | `ESP32_DISCONNECTED` | 「停止。モーター通信断」 | 1.2s | 継続中 10 秒毎 |
| A3 | E-Stop 発動（hw / tablet） | 「緊急停止」 | 0.7s | エッジのみ |
| B1 | `PERSON_TRACKER_LOST` → IDLE 強制 | 「見失い。待機へ」 | 1.0s | 継続中 15 秒毎 |
| B2 | heartbeat 途絶 → MANUAL 終了 | 「通信断。手動終了」 | 1.2s | エッジのみ |
...
| N23 | `lost_reason=DETECTION_LOST` | 「ロスト」 | 0.5s |
...
- **プランナ状態の可観測性（両レイヤの前提作業）**: 現状、追従ロジックの内部状態はどこにも publish
  されていない。`follow_planner_mapless` は `reason` をログ出力するのみ、`follow_planner` は状態出力を
  一切持たない。**追従系ノードが状態を publish するトピックの新設が先行する**。音声そのものより、
  この作業が本体となる
  - **安全通知レイヤ**: C3（捜索打ち切り）・C4（退避不可）・C7（障害物停止）・C8（自己位置なし）が
    該当する。残る 9 件（A1〜A3・B1〜B3・D1〜D3）は既存の購読のみで実装できる
  - **デモ実況レイヤ**: N6・N9〜N21 を中心に大半が該当する
  - 流用可能な既存トピック: `/person/search_mode`（Bool）、`/panel_navigator/arrived`
  - ただし `/person/search_mode` は Bool のため、予測中・捜索中・打ち切り後の 3 状態を区別できない。
    C3・N25・N26 を正しく鳴らすには 3 状態を表現できる形への置き換えが必要
...
- **周囲への予告手段（ブザー・ランプ）**: 音声は試験員の手元でのみ鳴るため、ロボットが動き出すこと
  ...
  ランプ側は本項目に引き継ぐ
- **音声のライセンス確認**: VOICEVOX は「VOICEVOX:キャラクター名」のクレジット表記を条件に利用できる
  が、各キャラクターには個別の利用規約があり「そのキャラクターとして扱う」ことが前提となっている。
  オリジナルキャラクターの声として既存キャラの音声を用いる構想（§7.5）は規約との整合確認が必要。
  選定した話者の規約原文を確認し、以下のいずれかで整理する。
  1. クレジットで借りている形にする（ビジュアルは自作、声は「VOICEVOX:○○」と明示）
  2. 規約の異なる別エンジン（AivisSpeech・COEIROINK 等）を使う
  3. 話速・音高・抑揚の調整で寄せる（1 との併用前提）
  - あわせて、生成した WAV をリポジトリに含めることが「音声の再配布」に当たるため、
    リポジトリが公開設定の場合は配布条件の確認が必要
...
| 14 | 音声フィードバック（安全通知） | 📋 方針確定・未着手 | §7。13 件中 9 件（A1〜A3・B1〜B3・D1〜D3）は既存購読のみで WebUI 完結。残る C3・C4・C7・C8 は状態 publish が前提（§7.8） |
| 15 | 音声フィードバック（デモ実況） | 📋 方針確定・未着手 | §7。大半が追従系ノードの状態 publish トピック新設に依存（§7.8）。行 14 の残り 4 件と同一の作業 |
