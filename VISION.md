...
- **地図作成停止後の自己位置推定**: 上記「試験場の地図運用」の要件を満たす方式が未決定。選択肢は 3 つ。
  1. **停止しない（常時マッピング）**: 実装ゼロ。常に自己位置補正が効き、ループクロージャで地図も改善し続ける。難点は、待機中に人が動き回ると slam_toolbox のポーズグラフに取り込まれて壁として焼き付くこと。「停止」ボタンの意味自体を運用から外すことになる
  2. **停止時に slam_toolbox の localization モードへ切り替える**: 正攻法。`serialize_map` でポーズグラフを保存し、`localization_slam_toolbox_node` に差し替える。localization モードはグラフにノードを追加せずスキャンマッチングだけ行うため、要件に厳密に合致する。難点は、実行中にノードを差し替える必要があり ROS2 の launch 構成では扱いが重いこと
  3. **保存して AMCL に切り替える**: `bringup.launch.py` に `map_yaml` 指定の経路が既にある。難点は起動時の選択であり走行中に切り替えられないこと
  現状は 1 を暫定運用としている。恒久方式を決めたらこの項目を該当セクションへ昇格させる
...
- **試験場の地図運用**: 実機は毎回起動時に SLAM で新規に地図を作成する（方針変更・2026-07-23）。事前に地図を保存して使い回す運用、および複数試験場の地図を切り替える UI は設けない。地図作成の開始・停止は WebUI から明示的に操作する: 起動直後は地図作成を停止した状態にし、試験場に到着し試験員の準備ができた時点で WebUI の「地図作成開始」で SLAM マッピングを開始、必要な範囲を走行後「地図作成停止」で確定する（`slam_control` ノードが `slam_toolbox` の `pause_new_measurements` をトグル。IDLE/MANUAL 中のみ操作可）。理由: 地図の保存・再利用・複数地図の管理を試験員だけで安全に完結させるのは技術的に難しいため、複雑な地図管理そのものをなくす方向に倒した。この変更はシミュレーション（`gazebo.launch.py` のシナリオ設定）には適用しない。事前地図によるテスト高速化のため `panel_shuttle`/`wide_area` シナリオは従来どおり `slam:false` + 保存済み地図を使い続ける
  - **「地図作成停止」の要件（2026-08-07 明確化）**: 停止が意味するのは「**地図の更新を止める**」であって「自己位置推定を止める」ではない。停止後も試験場内で待機・呼び寄せ（`SUMMONING` は `base_link→map` TF で試験員位置を変換する）・配電盤移動を行う以上、`map→odom` は走行中ずっと更新され続けなければならない。
    - **現状の実装はこれを満たしていない（未解決）**: `slam_control` が呼ぶ `slam_toolbox` の `pause_new_measurements` は、スキャン処理そのものを止める機能である（`shouldProcessScan()` が `isPaused(NEW_MEASUREMENTS)` を見て早期 return し、`map_to_odom_` は処理経路内でしか更新されない）。したがって停止後は `map→odom` が最後の値のまま凍結し、ロボットは純粋なデッドレコニングで走る。2026-08-07 の実機で、停止後に中速走行させると地図と自己位置が大きくズレることを確認し、`tf2_echo map odom` が完全に凍結していることを確認した。停止時点の `map→odom` は yaw -28.9°・並進 1.9 m あり、これはセッション中にオドメトリが蓄積した誤差量がそのまま補正凍結後の誤差成長率になることを意味する
    - **暫定運用**: 走行中は地図作成を停止しない。停止するのは駐機中だけにする
    - **恒久対処は未決定**（下記「地図作成停止後の自己位置推定」参照）
...
| 13 | 地図作成の開始/停止（WebUI） | ⚠ 要設計変更 | `slam_control` ノードが `slam_toolbox` の `pause_new_measurements` をトグルして仲介。起動直後は停止状態。IDLE/MANUAL 中のみ操作可。**ただし停止すると自己位置推定ごと止まり、以降はデッドレコニングになる**（2026-08-07 実機確認）。§8「地図作成停止後の自己位置推定」で恒久方式を検討中。暫定運用は「走行中は停止しない」 |
