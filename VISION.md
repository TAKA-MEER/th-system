...
#### Nav2 の機体モデル ── 円形 `robot_radius` を実測ベースで 0.40 に

**方針転換（2026-09-10 ユーザー決定）**: `nav2_params.yaml`（実機用のみ。`nav2_params_sim.yaml` は対象外）の
`local_costmap` / `global_costmap` 双方の `robot_radius` を **`0.36` → `0.40`** に変更する。

**経緯**: `robot_radius: 0.36` の算出根拠は「機体を 0.5m×0.5m の正方形とみなした外接円
√(0.25²+0.25²)=0.354→0.36」というコメントだったが、`registry.yaml` の `body_width_m` /
`body_length_m`（2026-08-18 に巻尺で実測した実際の最外形）は **0.55m×0.55m** であり、算出の
前提寸法自体が誤っていた。実測 0.55m×0.55m の外接円半径は √(0.275²+0.275²) ≈ **0.389m** で、
`0.36` はこれより狭く、**超信地旋回時の安全側マージンが実際には不足していた**（回転すると機体の角が
円の外に出る）。切り上げて `0.40` とする。

**矩形 `footprint` は検討したが採らなかった**: 実測寸法どおりの矩形（半幅 0.35m・半長 0.275m）も
一度試したが、RPP（`RegulatedPurePursuitController`）の衝突判定は **`footprint` を polygon で
指定すると外周が内接帯（253=INSCRIBED）に触れただけで停止**する（円形 `robot_radius` 指定時は
実際の壁セル 254=LETHAL に触れるまで許容する）。この差で、矩形にすると狭い直進通路の自動走行下限が
むしろ悪化した（0.75m 幅通路で「経路は確保できたがローカル衝突判定で数十cm 進んで停止」を実機ログで
確認）。円形 `0.40` は旧 `0.36`（回転で不足）と矩形（狭所で悪化）の両方より良い:

| モデル | 直進通路の自動走行下限（理論 / 実用） | 超信地旋回・直角カーブ |
|---|---|---|
| 旧 円形 0.36 | 0.72m / ~1.0m | 不足（要 0.389m） |
| 矩形 0.275×0.35 | 1.25m / ~1.5m | 十分だが直角カーブは中心が壁から 0.72m 必要で厳しい |
| **円形 0.40（採用）** | **0.80m / ~1.0m** | 十分（0.40 > 0.389）。中心が壁から 0.40m |

**代償**: 壁に正対して寄るとき（配電盤ドッキング）機体中心が壁から 0.40m までしか近づけない（実寸の
前後は 0.275m）。ただし配電盤前の向き合わせ（`ALIGN` の超信地旋回）はどのみち 0.40m の回転余裕を
要するため実質差は小さい。**ピン（配電盤位置）は壁から 0.45m 以上離して登録する前提**とする。

**機能制約（`robot_radius: 0.40` 前提。数値は実機で要確認）**:

- 直進通路: 自動走行の理論下限 0.80m、実用は 1.0m 前後（壁が costmap 上で厚く出る・自己位置誤差・
  追従誤差のぶん）
- 直角カーブ（L 字）: 各脚 1.0〜1.2m 以上。進路変化 45° 超で `rotate_to_heading`（その場停止＋旋回）
  が発動するので、交差部で全方向 0.40m 以上のクリアランスが要る
- ピン周辺: 0.45m 以上のクリア空間。ピン近傍 0.5m 以内が全て通行不可だと経路計画が BLOCKED
