...
- 安全通知に必要なトピックは WebUI が既に購読済み（`/robot/mode`・`/safety/fault`・`/safety/estop`・
  `/person/status`）であり、新規 ROS2 ノード・新規購読を要しない
...
- **プランナ状態の可観測性（デモ実況の前提作業）**: 現状、追従ロジックの内部状態はどこにも publish
  されていない。`follow_planner_mapless` は `reason` をログ出力するのみ、`follow_planner` は状態出力を
  一切持たない。デモ実況レイヤ（特に N6・N9〜N21）の実装には**追従系ノードが状態を publish する
  トピックの新設が先行する**。音声そのものより、この作業が本体となる
...
| 14 | 音声フィードバック（安全通知） | 📋 方針確定・未着手 | §7。WebUI で完結。必要トピックはすべて購読済みのため追加の ROS2 側作業なし |
| 15 | 音声フィードバック（デモ実況） | 📋 方針確定・未着手 | §7。追従系ノードの状態 publish トピック新設が先行作業（§7.8） |
