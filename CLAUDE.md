...
- **テストの大半は Docker 不要でホストの `python3` から直接走る。**`th_ws/src/th_testing/test/` のうち ROS2 環境（`rclpy` / ビルド済み `th_system_msgs`）が要るのは次の 10 ファイルだけで、他は素の pytest で緑赤を判定できる（2026-08-26 時点で 420 passed / 1 skipped）。`colcon build` は数分かかるので、まずホストで回して最後に Docker で 1 回通すのが速い。
