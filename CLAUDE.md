...
- **テストの大半は Docker 不要でホストの `python3` から直接走る。**`th_ws/src/th_testing/test/` のうち ROS2 環境（`rclpy` / ビルド済み `th_system_msgs`）が要るのは次の 22 ファイルだけで、他は素の pytest で緑赤を判定できる（2026-09-25 時点で 1148 passed / 1 skipped）。**ROS が要る試験を足したらこの一覧にも足すこと**（2026-09-23〜25 に 7 件が載らないまま増え、各担当が除外漏れを踏んだ）。`colcon build` は数分かかるので、まずホストで回して最後に Docker で 1 回通すのが速い。
  除外する 22 ファイル: `test_connectivity_checker_node.py` / `test_fault_detection.py` / `test_mode_transitions.py` / `test_params_audit_node.py` / `test_safety_monitor.py` / `test_state_manager_node.py` / `test_twist_mux_priority.py` / `test_simulation_scenarios.py` / `test_msg_definitions.py` / `test_esp32_bridge_node.py` / `test_jog_gate_node.py` / `test_localization_lost.py`（WP-SAFE-05） / `test_runaway_freshness_node.py`（W-06 の②） / `test_planned_restart_node.py`（O-e3） / `test_slam_control_restart.py`（O-e3。rclpy 要・launch なし。後ろ 6 つのうち本ファイルを除く 5 つは `launch_testing`） / `test_localization_mode_gate_node.py`（WP-SAFE-05） / `test_dr_spaam_lifecycle_controller_node.py` / `test_person_tracker_bridge_node.py` / `test_wait_clear_gate_node.py`（以上 8-c） / `test_esp32_scale.py`（9-a） / `test_opcheck_runner_node.py`（10-a） / `test_dev_mode_safety_node.py`（開発モード）
...
- **実機の bringup が `th_robot` で動いている間に `docker compose run` で試験を回すと、DDS で混線して関係ない試験が落ちる。**compose は `network_mode: host`・`ROS_DOMAIN_ID=10` なので、試験用コンテナも実機と同じドメインに入り、実機の `/scan`・`/safety/estop_hw` 等が試験に混ざる。`connectivity_checker_node`（`evt.link_ok が一度も出ない`）・`fault_injection_05`・`test_fault_detection` が落ち、1 本ずつ再実行しても直らない（2026-09-23）。**`docker compose run -e ROS_DOMAIN_ID=77 ...` のようにドメインを分ける**と全部通る。
- **`colcon test` の失敗数だけを見て回帰と判断しない。比べるのは件数ではなく落ちたファイルの集合**。2026-09-25 時点の `main` で落ちるのは `fault_injection_02/04/06/08/09/10`（04/08/10 は実機専用・未実施）・`test_safety_monitor`（`test_esp32_fault_cleared_on_recovery`）・`test_state_manager_node`（`test_fault_estop_still_idle_only`・`test_latch_roundtrip` の 2 メソッドが毎回赤。WS-9O の仕様変更に試験が追いついていない疑い）。`test_state_manager_node` を触ったときはメソッド単位で比べる。
