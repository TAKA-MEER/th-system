...
**AP を差し替える方向は 2026-08-20 に否定された**（[機体作業手順-段階0](../../機体作業手順-段階0.md) §4.0）。

| 実測 | 内容 |
| --- | --- |
| 同一条件で LiDAR は無傷 | 同じ電波・同じ受信機・同じ 10 分で LiDAR は max 104.2 ms（取りこぼしゼロ）、ESP32 だけ max 603.4 ms |
| 無線区間は健全 | ESP32 AP へ ICMP 5 Hz × **600 秒（3000 発、同じ 10 分）**: **損失 0 %、max RTT 33.2 ms、100 ms 超 0 件**。記録: `data/meas04/ping_icmp_600s.md` |

**片方の経路だけが悪いなら原因は電波ではない。**このパケットの作業は
「AP 3 案の比較」ではなく、**ESP32 経路（TCP／ファーム／`esp32_bridge`）の跳ねを潰すこと**に読み替える。
3 択の切り分けは `th_ws/scripts/meas05_esp32_triage.sh`。
