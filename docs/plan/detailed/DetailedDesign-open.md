...
| # | 対象 | 内容 | 状態 |
| --- | --- | --- | --- |
| **A-1** | `Spec-modes.md` §3.1.1 / §3.1.2 | **表に ID 列を足す**（`SM-3.1.1-01` / `SM-3.1.2-034`）。行番号で参照すると 1 行編集で全 `spec_ref` がずれる | **未対応** |
| **A-2** | `Spec-modes.md` §3.1.2 | 詳細設計にしか無い遷移を戻す。`T-INIT-03` / `T-TEACH-06` / `T-PREP-04` / `T-PREP-08` / `T-PREP-09` / `T-REPLAY-03` / `T-SUM-08`〜`10` / `T-SUM-13` / `T-OPC-04` / `T-OPC-06` / `T-OPC-08` / `T-LINE-05` / `T-LINE-06` / `T-LEASH-07` / `C-06r` / `C-09f` / `C-15` | **未対応** |
| **A-3** | `Spec-modes.md` §4.1 | 「物理押下の例外は始業点検の 1 つだけ」に **`INIT/CHECK`（起動時に押されたまま）** を加える（`CL-B-6` は本文にあるが遷移の例外として書かれていない） | **未対応** |
| **A-4** | `Spec-modes.md` §3・§3.1.2 | `PANEL_NAV` / `SUMMON` / `HOME_NAV` の **`ARRIVED` は滞在する状態ではない**（通過の注記）と分かる表記に直す。状態として持つと到達不能状態になる | **未対応** |
| **A-5** | `Spec-safety.md` §3.5 | 重大フォルトの列挙に **`LOCALIZATION_LOST`** を足す | **未対応** |
| **A-6** | `Spec-safety.md` §6.1 | 「必須通信系統の断 → 動作系モードは **`IDLE` へ強制遷移**」が §3.5（F-20）と矛盾したまま残っている。**`PAUSE` に直す** | **未対応** |
| **A-7** | `Spec-open.md` C-15 | 「`SUMMON` の `PERSON_TRACKER_LOST` は `IDLE` へ強制遷移」が `Spec-modes.md` §5（`PAUSE`）と食い違う。F-20 の決定に合わせる | **未対応** |
| **A-8** | `Spec-modes.md` §7 ／ `mockup/index.html` | **ゾーンを 3 値にする。**モックアップの `SCREENS[].zone` は真偽値で `NA` を場外に潰しており、`OPCHECK` / `CALIB` に `v_max` が適用される | **未対応** |
| **A-9** | `Spec-modes.md` §7 | ゾーンの合成を「強弱」ではなく **「速度上限の最小値」** に直す。`NA` の画面（S-30/31/40）は `IN` より低い上限を持つ | **未対応** |
| **A-10** | `Spec-modes.md` §6 | `OPCHECK` / `CALIB` / `IDLE` は `PAUSE` を持たない。§3.1.1「`PAUSE` を持たないモードは無い」の例外として明記する | **未対応** |
| **A-11** | `Spec-checks.md` §3.6 | `O-d10` を「未確定」から「解決済み（スケール係数方式）」へ | **未対応** |
...
| **12** | R2-15 | **詳細設計にしか無い遷移 17 行を正本へ戻す** | **未対応** → §3 A-2 |
...
| **16** | R2-30 / R2-31 / R2-37 | 正本の矛盾（`Spec-safety.md` §6.1 と §3.5、`Spec-open.md` C-15 と `Spec-modes.md` §5、`LOCALIZATION_LOST` の位置づけ） | **未対応** → §3 A-5 / A-6 / A-7 |
...
| **2** | 実装-2 ／ 安全-16・17 | `spec_ref` の実値が 1 行も入っていない | **未対応。**正本への ID 列追加（A-1）が前提。§3 参照 |
...
| **9** | 網羅-1・2 | `O-d5` / `CL-M-7`（物の配置が変わる前提の地図再利用可否） | **未対応。**§4 の未確定へ移す |
| **10** | 網羅-3・4 | `F-18` / `T-r11` / `T-r12` / `DF-D-7`（ハードウェア構成表） | **未対応。**§3 の申し送りへ移す（機材選定は `WP-LINE-00`） |
...
| # | 出所 | 指摘 |
| --- | --- | --- |
| **A** | 実装-2 ／ 安全-16・17 | **`spec_ref` の実値。**正本 `Spec-modes.md` §3.1.1 / §3.1.2 への ID 列追加（§3 A-1）が前提。A-2 の列挙に 6 件（`T-PNAV-07` / `T-HNAV-05`〜`07` / `T-ATP-05` / `C-14`）と `CALIB` 行の修正が漏れている。**`WP-STATE-01` の着手前提条件** |
| **B** | 網羅-1・2 | **`O-d5` / `CL-M-7`**（前日と当日で物の配置が変わる前提の地図再利用可否）が 1 度も出てこない |
| **C** | 網羅-3・4 | **`F-18` / `T-r11` / `T-r12` / `DF-D-7`**（ハードウェア構成表へのカメラ・リードデバイスの追加）に対応する節が無い |
| **D** | — | **段階 3〜8 のパケットに実体が無い。**着手前に `DetailedDesign-wp<n>.md` を書く（[packets](DetailedDesign-packets.md) §0.2） |

**3 回目のレビューは、A〜C を片づけてから行う。**D は段階 3 に着手する時点でよい
