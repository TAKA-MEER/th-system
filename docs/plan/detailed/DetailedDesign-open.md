...
| ~~DEBT-2~~ | ~~`blind_angle_ranges` が幅ゼロ~~ → **【解除・2026-09-09】** 走行体のみの構成では死角なしと確認し空配列で確定していた（`status: measured` / `value: []`）が、上部構造（四隅の支柱）搭載後に `scripts/measure_blind_sectors.py` で実測。周囲 500mm 以内の実体を除去した状態で `/scan` を 300 スキャン集計し、支柱 4 本を laser 中心から約 0.33m の 4 セクタ（前左 +45.9..+54.9 / 前右 -51.9..-45.4 / 後左 +130.3..+138.8 / 後右 -132.8..-125.8）として検出。`registry.yaml` の `blind_angle_ranges` に平坦配列で格納（`lidar_filter` / `obstacle_limiter` の両方が消費）。4 セクタはいずれも `obstacle_limiter` の前方コーン(±28.6°)・後方コーン(±34.4°)の外なので L5 の `v_reverse` キャップは前後移動には発火しない。**次回 launch で生成物に反映され死角マスクが有効になる。** WP-CALIB-01 §10 ④ の `/scan` vs `/scan_filtered` 検証は実機再起動後に実施すること | 実測済み（`registry.yaml` / `scripts/measure_blind_sectors.py`） | 段階 2（解除済み） |
