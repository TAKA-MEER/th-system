...
| **N-5** | **ガード不成立時の `reject_reason_key` が決まっていない。**[state](DetailedDesign-state.md) §3 のスキーマは「**ガード不成立時**、または `reject: true` のとき」返すと定義しているのに、**§3.1 の 6 手順にはガード不成立時の手続きが無く**、§4.1・§4.2 の遷移表にも `reject_reason_key` 列が無い（`C-06r` だけがインラインで持つ）。結果、**§10 に並ぶ 19 キーのうち 17 キーは表から到達できず、すべて `not_allowed` に落ちる。**安全上の欠陥ではない（拒否は拒否として成立し、`S-3` も満たす）が、**UI が「なぜ拒否されたか」を言えない**ので `SD-3` の意図を損なう | **未決**（`WP-STATE-01` で発見。実装は §3.1 の 6 手順どおり `not_allowed` に倒してあり、キーを推測で割り当てることはしていない）。**決めるべきは「どのガードが偽のとき、どのキーを返すか」の対応表**で、これは設計判断であって実装判断ではない。`WP-STATE-02`（`state_manager`）と `WP-UI-01` の前までに決める |
| **N-6** | **根拠が無い `class: b` の方針値を表現する手段が無い。**`S3` は `class: b` に `derived` か `given` しか許さず、`placeholder`（＝根拠が無い暫定値・UI にバッジ）を使えない。結果、**`WP-PARAM-01` で 43 行が「暫定値。実機データ未確認」という `note` だけを付けて `given` になった**（`safety_margin_m` / `floor_margin_m` / `w_max` / 機体寸法など、安全側の閾値を含む）。`note` は機械判定されないので、**UI のバッジにも `params_status` の件数にも出ない** | **未決**（`WP-PARAM-01` で発生）。選択肢は ① `class: b` にも `placeholder` を許して `S3` を緩める（伝播規則 `S5` はそのまま使える）／② `provisional` のような 5 つ目の `status` を足す／③ 43 行すべてを人が決めて `given` の根拠を書く。**②か③が要るのは「人が決める方針値」であって測定値ではない**ので、`WP-MEAS-*` では埋まらない |
| **N-7** | **`params.md` §6.1・§6.3 にあって `names.md` §7 に無いパラメータが 12 件。**`fault_to_stop_ms` / `wheel_base_m` / `wheel_radius_m` / `camera_lift_tolerance_m` / `camera_lift_tolerance_deg` / `panel_count` / `panel_spacing_m` / `panel_front_space_m` / `prep_baseline_min` / `floor_type` / `unmanned_permission` / `carry_baseline_min`。`names.md` §7 が**パラメータ名の正**なので、`registry.yaml` からは除外した（`WP-PARAM-01`） | **未決**。①`params.md` 側が古いなら該当行を消す／②`names.md` §7 に追記して `registry.yaml` へ戻す、のどちらか。**`wheel_base_m` / `wheel_radius_m` は校正（`WP-MAINT-02`）が使う**ので、②になる可能性が高い |
| **N-8** | **`body_half_length_m` が `body_length_m / 2` の値を `given` として二重に持っている。**`floor_distance()` / `clear_distance()` の入力はこちらなので、**全長だけを直すと足元距離が古い機体寸法のまま**になる（2026-08-18 の実測反映時に実際に踏みかけた）。`DD-6`（1 つの数値を 2 か所に持たない）に反する | **未決**。本来は `status: derived` にして `derived_from: [body_length_m]` から計算すべきだが、`derive.py` に「半分にする」関数が無く、`params.md` §3 の 12 関数にも入っていない。**関数を 1 つ足すか、`body_half_length_m` を廃して各所で `body_length_m / 2` を計算するか**を決める。当面は `note` に警告を書いて運用する |
| **N-9** | **エンコーダのカウントをソフトウェア割り込みで行っており、WiFi の受信ギャップ（p99）を悪化させている。**`esp32/src/encoder.cpp` は 4 本のピンすべてに `attachInterrupt(..., CHANGE)` を張っている。`ENC_COUNTS_PER_REV = 4096`・駆動輪の周長 0.126 m から、**0.5 m/s で毎秒 約 32,500 回、1.0 m/s で 約 65,000 回**の ISR が走り、WiFi タスクを不定期に横取りする。**ESP32 には PCNT（パルスカウンタ）というハードウェア周辺機能があり、直交エンコーダを CPU を使わずに数えられる**（入力専用ピン GPIO34〜39 でも使える） | **未決**（2026-08-18 発見）。**速度改善の本命候補。**[safety](DetailedDesign-safety.md) §7 の 2 本制約により `v_max ≤ intrusion_budget_m / (p99 × 余裕率)` なので、**p99 が下がればそのまま最高速度が上がる**（`docs/plan/task.md` 2「追従走行の速度を改善」）。AP の選定は電波環境の改善だが、**こちらは MCU 内部の確定的な改善**である点で性質が違う。**2026-08-18 に実測し、0.5 m/s では有意差が無いことを確認した**（車輪停止 p99 140.6ms → 0.5m/s 回転 139.5ms。LiDAR 側も p99 の 95 パーセンタイルで 206.5 → 204.8ms と逆転）。**優先度を下げる。**使用マイコンは 2 コアで無線処理と割り込みが競合しない構成か、1 回の割り込みが想定より軽いと考えられる。**ただし『未検証』ではなく『0.5 m/s では差が無い』という実測付きの保留**であり、**1.0 m/s 以上では割り込み頻度が倍になる**ので、速度域を上げた時点で再確認する。測定データ: `docs/plan/detailed/data/meas04/esp32ap_static.csv` / `esp32ap_spin.csv` |
| **N-10** | **registry の空リストは ROS2 パラメータファイルで配れない。**`enabled_targets: []`（`safety_monitor` の監視対象）・`required_nodes: []`（`connectivity_checker` の疎通判定対象）はどちらも `status: given` の正当な値だが、ROS2 のパラメータ YAML には**型付きの空配列を書く手段が無い**（要素が無いと rcl の YAML パーサが型を決められず `declare_parameter` が `No parameter value set` で失敗する。しかも宣言していないノードの起動まで巻き込む）。`WP-PARAM-02` の生成後サニタイズ（`params_generation.sanitize_node_params()`）はこれを生成物から**落とすしかなく**、その結果ノード側のコード既定値（`declare_parameter` 呼び出し時の default）が使われる | **未決**（`WP-PARAM-02` で発見・回避策のみ実装。2026-08-19）。「registry が単一の出どころ」という原則がこの2つについては成立せず、**ノード側の既定が空リストであることに依存**する。決めるべきは (a) このまま「空はノード側の既定に任せる」と決めるか、(b) 空を表すセンチネル要素（例: 空文字列 1 個）を registry〜生成物間で渡す約束にするか |
