...
| **N-23** | **`mode_manager.cpp` が `/safety/fault` の `severity` を見ておらず、`CRITICAL` フォルトでも `ESTOP` ではなく `IDLE` にしか遷移しない。**`sub_fault_` のコールバック（`mode_manager.cpp`）は `msg->fault_type` だけで分岐し、アクティブなモードを常に `IDLE` へ強制遷移させる（`PERSON_TRACKER_LOST` の narrower gating のみ例外）。`ESTOP` へ遷移するのは `/safety/estop`（ハード/UI E-Stop）だけ。これは[safety](DetailedDesign-safety.md) §5.1 の「`CRITICAL` → `ESTOP`」という表と食い違う。**帰結**: 故障注入11（`obstacle_limiter` を SIGKILL）の4番目のアサーション（`/robot/mode` が `ESTOP` になること）は**現状失敗する想定**（`th_testing/test/fault_injection/case_11_limiter_sigkill.py`）。§5.2.1「`th_state` の `ESTOP` 遷移は表示と復帰のためだけであり、駆動を止める役目は持たない（駆動を止めるのは層3=`/safety/fault_lock`）」により、**駆動停止（`fault_lock`）という安全上の本質的な結果には影響しない**——`/safety/fault_lock = LIDAR_LOST || ESP32_DISCONNECTED || severity==CRITICAL` は `safety_monitor_core.cpp::compute_fault_lock()`で実装済みであり正しく動く。範囲外として意図的に手を付けていない（`WP-TEST-01` はテストファイルの実装が範囲であり、`mode_manager.cpp`の安全関連ロジック変更は別パケットが要ると判断した） | 未決（[safety](DetailedDesign-safety.md) §5.1・故障注入11） |
