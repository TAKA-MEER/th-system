...
| ~~DEBT-2~~ | ~~`blind_angle_ranges` が幅ゼロ~~ → **【解除・2026-09-09】** 走行体のみの構成では死角なしと確認し空配列で確定していた（`status: measured` / `value: []`）が、上部構造（四隅の支柱）搭載後に `scripts/measure_blind_sectors.py` で実測。周囲 500mm 以内の実体を除去した状態で `/scan` を 300 スキャン集計し、支柱 4 本を laser 中心から約 0.33m の 4 セクタ（前左 +45.9..+61.5 / 前右 -59.5..-38.9 / 後左 +130.3..+143.5 / 後右 -132.8..-117.5）として検出。`registry.yaml` の `blind_angle_ranges` に平坦配列で格納（`lidar_filter` / `obstacle_limiter` の両方が消費）。4 セクタはいずれも `obstacle_limiter` の前方コーン(±28.6°)・後方コーン(±34.4°)の外なので L5 の `v_reverse` キャップは前後移動には発火しない。**次回 launch で生成物に反映され死角マスクが有効になる。** WP-CALIB-01 §10 ④ の `/scan` vs `/scan_filtered` 検証は実機再起動後に実施すること。**追記（2026-09-09）**: 初回実測値（+45.9..+54.9 等）はグレージング角のビームが `measure_blind_sectors.py` の 90% persist 判定で切り詰められ、地図に支柱の点群が漏れて写る不具合が実機で見つかった。`/scan` と `/scan_filtered` をセクタ境界の内外で突き合わせて再実測し、上表の値に拡張して解消（詳細は `registry.yaml` の `blind_angle_ranges` note） | 実測済み（`registry.yaml` / `scripts/measure_blind_sectors.py`） | 段階 2（解除済み） |
...
| ~~N-8~~ | **`blind_angle_ranges` の「死角なし」と「未校正」を配列の形だけで区別できない。**[safety](DetailedDesign-safety.md) §4.4 は「全ペアが幅ゼロ → `obstacle_limiter` は AUTO の走行開始を拒否する」と定めるが、`params_generation.sanitize_node_params()` は D3 の設計どおり空リストをキーごと落とすため、生成物では「死角が無いと確認済みで空配列」と「値が抜けてバグで消えた」を区別できない。安全ゲートの判定を配列の形から推論するのは危険 | **解決**（2026-08-26）。走行体のみの構成では LiDAR に死角が無いことを確認済みなので `blind_angle_ranges` は `status: measured` / `value: []` で確定した。判定は配列の形からではなく、新設した `blind_calibrated`（`class: b` / `given` / `value: true`）という明示フラグで行う設計にした。既定を `false`（拒否）側にせず現状 `true` にしているのは、死角が実際に無いことを確認済みであるため。上部構造の追加などで死角ができたら、`WP-CALIB-01` の手順で `blind_angle_ranges` を再測定するのと同時に `blind_calibrated` を明示的に見直すこと（`registry.yaml` / [names](DetailedDesign-names.md) §7.2）。**追記（訂正・2026-08-27）**: 上記解決策の一部だった「サニタイズが落とした `blind_angle_ranges: []` を `reshape_blind_angles()` で明示的に書き戻す」処理は**誤りだった**。Docker 実起動で `obstacle_limiter`・`lidar_filter` の両方が `parameter_value_from failed for parameter 'blind_angle_ranges': No parameter value set` で起動失敗することを実測で確認した——原因は `declare_parameter` の型推論ではなく **ROS2 Humble の `rcl_yaml_param_parser` が空配列の parameter override をそもそも解決できない**こと（素の `tf2_ros::static_transform_publisher` でも同じ例外で落ちることを実測で確認済み。`ParameterDescriptor` で型を明示しても防げない）。`reshape_blind_angles()` は撤去し、`sanitize_node_params()` の一般則（空配列のキーは丸ごと落とす）をそのまま適用するようにした。**「死角なし」と「未校正」の区別は `blind_calibrated` だけで十分**なので（`blind_angle_ranges` というキー自体が生成物に存在しなくても意味は失われない）、この訂正で N-8 の本来の解決方針（判定を明示フラグに委ねる）が損なわれることはない |
...
| **N-21** | **`WP-CALIB-01` §10 の完了チェックが、死角なしの構成では必ず失敗する。**[wp2](DetailedDesign-wp2.md):373 は完了条件に `grep -q "blind_angle_ranges" /root/th_data/generated/lidar_filter.yaml` を挙げているが、`blind_angle_ranges` が空（＝死角なしを確認済み）のときは**キーごと生成 YAML から落とす**のが正しい実装になった（ROS 2 Humble は params YAML の空配列を扱えず、rclcpp は `parameter_value_from failed ... No parameter value set` で落ち、rclpy は未初期化のまま `get_parameter().value` で例外になる。標準ノード `tf2_ros/static_transform_publisher` でも同じ現象を実測して確認した）。**したがって現構成ではこの grep は満たせない。**死角が実在する構成に戻ったときだけ通る条件である。**完了判定では代わりに `blind_calibrated` が生成 YAML に載っていることを見ること**（校正済みかどうかを運ぶのはこちらである ―― N-8 の決着）。N-19（`WP-SAFE-03` §10 の `cmd_vel_retreat` grep）と同種の陳腐化。**追記(2026-09-09)**: 上部構造搭載で `blind_angle_ranges` が非空（4 セクタ）に戻ったため、現構成ではこの grep は通る。ただし「空配列のときはキーごと落ちるので grep が満たせない」という陳腐化の本体は未解消（将来また死角なし構成に戻れば再発する）。完了判定は `blind_calibrated` を見る方針のまま | 未決（[wp2](DetailedDesign-wp2.md) `WP-CALIB-01` §10） |
| ~~N-22~~ | **`WP-SAFE-03` で `obstacle_limiter` を最終段に入れた結果、`state_manager` を起動しない `gazebo.launch.py` では `/system/state` が stale となり、シミュレーションでロボットが一切動かなくなっていた。**`obstacle_limiter_core.cpp` の `update()` は `/system/state` が `state_stale_ms`（既定1500ms）以内に更新され続けていないと `applied_limit_mps` を強制的に 0.0 にするが、`gazebo.launch.py` には `state_manager` の `Node` 定義自体が存在しなかった（`state_manager` は `bringup.launch.py` 専用）。実測で確認した: `ros2 topic info /system/state` の publisher count が 0、`/cmd_vel_nav` へ `linear.x=0.3` を送り続けても `ros2 topic echo /cmd_vel --once` は `linear.x=0.0`、`/safety/limiter_status` は `action: STOP` / `nearest_obstacle_m: 2.08`（障害物は2m先で無関係） / `source_class: AUTO` / `applied_limit_mps: 0.0`——障害物とは無関係に上限 0 だった。**対処**（`WP-TEST-01`。2026-08-27）: `gazebo.launch.py` に `state_manager` を追加した（`bringup.launch.py:283` と同一定義。純粋な Python ノードでハードウェア依存が無いため sim/実機共通で無条件起動）。ところがこれだけでは足りないことも判明した——`state_manager` は `INIT` から `IDLE` へ進むのに `evt.link_ok`（`connectivity_checker` だけが発行）を要り、`connectivity_checker` は(a) `/safety/estop_hw` を一度も受信していない間は不合格として扱うフェイルセーフを持つが、その唯一の publisher `esp32_bridge.py` は sim で起動しない、(b) LiDAR 項目の判定が `scan_points == scan_expected_points`の**完全一致**を要求するが、`registry.yaml` の `scan_expected_points`（実機 SLLIDAR 値）は **1080**、対して Gazebo の LiDAR センサ（`th_description/urdf/gazebo_plugins.xacro`）は `<samples>720</samples>`で一致しない。(a) は `gazebo.launch.py` の `connectivity_checker` を`sim:=true`（`connectivity_core.py` が Gazebo シナリオ向けに元々用意していたパラメータ）で起動しつつ、テスト側が `/safety/estop_hw=False` を発行して回避した（`th_testing/test/fault_injection/conftest.py` の`enter_manual_mode()`。実機に存在しない生の入力信号を代わりに供給するという判断で、`/ui/active_screen` と同種の扱い——`state_manager` が計算する**出力**である `/system/state` 自体は偽装しない）。(b) は**解決した（コーディネーター判断・2026-08-27）**。3案のうち `gazebo_plugins.xacro` の `<samples>` を実機値へ合わせる案を採った——「シミュレーションは実機を模すためのものであり、センサの点数が違うこと自体が模擬の不備。それに合わせて検査を緩めるのは本末転倒」「sim用に値を分けると `/system/state` を偽装しないという判断と同じ理屈で経路が分かれ試験の意味が薄れる」「疎通のみに緩めると安全検査（スキャンモード誤設定等の劣化を捉える）を sim で無効化してしまう。故障注入試験こそそれを通すべき」という理由による。`th_description/urdf/gazebo_plugins.xacro:57` の `<samples>720</samples>` を `<samples>1080</samples>` へ修正した（角度範囲は元々 `-3.14159..3.14159` のため 2π/1080≈0.333°/点で実機と一致する。コメントの「0.5°分解能」も修正した）。実害はスキャン1本あたりの計算量が1.5倍になる程度で無視できる、という判断。他に `720` がLiDARの点数として決め打ちされている箇所が無いか `th_ws/src/` 全文検索で確認した——`th_safety/test/test_obstacle_limiter_core.cpp` の合成スキャン（720点、自己完結した純粋関数テスト用データで Gazebo の実センサとは無関係）以外に実害のある箇所は無かった。**この修正により `state_manager` は sim でも `INIT` から `IDLE` へ進めるようになった（Docker での実機確認はコーディネーターが別途実施）**。**さらに** UI が繋がっていない sim では、たとえ (b) が解消しても `/ui/active_screen` を誰も発行しなければ `speed_limit="stop"` のままなのは**設計どおりのフェイルセーフ**であり（`names.md` §4.1「`/ui/active_screen` が途絶／使用中が 0 台 → 停止」）、これは変えてはいけない——**試験側が `/ui/active_screen` を発行して「操作画面が使用中」を作ることで実連鎖を通す**設計にした（画面 `S-11`「手動走行」。`v_max`・`needs_tracker: unused` のため教示前提を持ち込まずに済む） | **解決**（2026-08-27。`gazebo_plugins.xacro` の `<samples>` を1080へ修正） |
| ~~N-23~~ | **【訂正・2026-08-28】当初「`mode_manager.cpp` が `severity` を見ず `CRITICAL` でも `IDLE` にしか遷移しない」としていたが、これは誤りだった——**新設計の FSM（`th_state` の `state_manager`）は正しく `ESTOP` へ遷移している。**コーディネーターが Docker で実測（`ros2 topic echo` で `/safety/fault` と `/system/state` を先に記録開始）したところ、`/safety/fault` は `active: true` / `fault_type: LIMITER_DEAD` / `severity: CRITICAL` を示し、`/system/state.mode` は `INIT` → `IDLE` → `MANUAL` → `ESTOP` と正しく遷移した（`safety_monitor` → `severity: CRITICAL` → `fault.critical` イベント → `transitions.yaml` の `C-06a`（`mode:'*'` / `event: fault.critical` → `to_mode: ESTOP`）という連鎖がすべて働いている）。**真因はテスト側にあった**: `th_testing/test/fault_injection/case_11_limiter_sigkill.py` が `/robot/mode`（`RobotMode` 型・数値定数）を購読していたが、これは **as-built の旧FSM `mode_manager.cpp` が publish する廃止予定のトピック**であり、`WP-TEST-01` の仕様 §3 の観測点表が指定する観測対象は `/system/state` の `mode`（**文字列**）である——`WP-TEST-01` §3 で確認しておくべきだった、テスト側の観測対象の誤り（既に修正済み。`/system/state.mode` の文字列 `"ESTOP"` を見るように直し、`conftest.py::assert_equals_within` の docstring も修正した）。**なお `mode_manager.cpp`（as-built の旧FSM）自体は 依然として `/safety/fault` の `severity` を見ず、`CRITICAL` フォルトでも `IDLE` にしか遷移させない**——これは新FSMへの移行が段階3で完了するまでの 過渡期の既知事項であり、`/robot/mode` を実際に消費している `follow_planner`/`panel_navigator`/`manual_command_handler` 等の as-built ノードが残っている間は実害になりうるが、`WP-TEST-01` の観測対象ではないため このパケットの範囲では追跡のみ**（旧FSM側の修正は別途要判断） | 解決（2026-08-28。テスト側の観測対象を `/system/state` へ修正）。旧FSM `mode_manager.cpp` の severity 対応漏れ自体は残存（別途判断要） |
| **N-24** | **`SAFETY_ENABLED_TARGETS`（`gazebo.launch.py` / `bringup.launch.py`）に、対応する publisher が既に揃っている監視対象が抜けている疑いが複数ある。**`limiter`（`N-22` の元の話。obstacle_limiter が publish しているのに未有効化だった）を修正した際、コーディネーターが同種の取りこぼしを**当日だけで10件**発見している。判明済みの未検証項目: `state`（`WP-TEST-01` で `state_manager` をsim に追加したが、`STATE_INCONSISTENT` 検出は起動直後のモード遷移シーケンス（`INIT`→`IDLE`→`MANUAL`）中に誤検知しないか未検証のため意図的に見送った）・`mux`（`/cmd_vel_muxed` の remap 先変更は `WP-SAFE-03` で完了済みだが、`MUX_DEAD` 検出は未有効化のまま。故障注入12の担当範囲と判断し今回は触れていない）。**`enabled_targets` を触る作業（新しい publisher を追加する・既存の publisher の配線を変える）をするたびに、この表の全項目を publisher の実在と突き合わせて見直すこと**（`limiter` のような「後で有効化するはずが忘れられる」パターンの再発防止） | 未決（`SAFETY_ENABLED_TARGETS_SIM` / `SAFETY_ENABLED_TARGETS_REAL` / `SAFETY_ENABLED_TARGETS`。[safety](DetailedDesign-safety.md) §5.2） |
| **N-25** | **故障注入 6「フォルト検知 → 100ms 以内に停止」が Gazebo で合格しない。**[safety](DetailedDesign-safety.md) §10 #6 の合格条件（層 3 の応答時間 100ms）に対し、`/cmd_vel_muxed` が期限内にゼロにならない。**安全挙動そのものは検証できている**——故障注入 5（`LIDAR_LOST` の発火）と 11（`LIMITER_DEAD` → `ESTOP` → 駆動ゼロ。ただし判定窓は `limiter_dead_ms×10 = 2500ms`）は**両方とも通過**しており、「通信断でフォルトが立ち駆動が止まる」ことは担保されている。満たせないのは**100ms という時間の上限**だけである。**試した対処と結果**（すべて Docker 実測）: 駆動周期 10Hz→50Hz（`af77b9c`）→ 不合格のまま。`DriveController` を rclpy タイマーから専用スレッドへ（`a3ad468`。`_spin_until` の外ではタイマーが発火しない件の対処）→ 不合格のまま。**未解明の点**: (1) 計測は `time.monotonic()`（実時間）で行うが、被計測系は `use_sim_time` で動く。この環境の実時間係数は 0.14〜0.3 と低く、cut 後は合成クロックが1:1 で進むため係数が不連続に変わる。**実時間の物差しでシミュレーション時刻の予算を測っている**ことが妥当かは未検証。(2) 100ms を超えるのが計測の artifact なのか実際の遅延なのかを切り分けていない。**次にやるなら**: `/safety/fault_lock` が true になった時刻から `/cmd_vel_muxed` がゼロになるまでの実測値を（合否ではなく）数値として出し、100ms をどれだけ超えるかを把握するところから。実機（実時間係数 1.0）でも同じかを確かめる価値がある | 未決（[safety](DetailedDesign-safety.md) §10 #6） |
| **N-26** | **故障注入 12 の合格条件「`cmd_vel_stale_ms` 以内」は、そのままでは正しい実装でも必ず落ちる。**[safety](DetailedDesign-safety.md) §10 #12 は「`obstacle_limiter` を止めたまま非ゼロ指令を残し、`cmd_vel_stale_ms` 以内に ESP32 への指令がゼロ」と定めるが、`esp32_bridge` の stale 判定（`keepalive_core.keepalive_value()` の `now - last_cmd > stale_ms`）は 20Hz のキープアライブタイマー（`esp32_bridge.py` の `create_timer(0.05, ...)`）でしか評価されない。したがってゼロ化の実測は原理的に `(cmd_vel_stale_ms, cmd_vel_stale_ms + 50ms]` に量子化され、`<= 400ms` という厳密解釈は実装が正しくても成立しない。**Docker 実測**（健全時 3 連続）: **441.6 / 405.5 / 415.8 ms**——予測どおり量子化窓にほぼ一様に散らばった（予算を 2 周期へ広げた後の 5 回も 445.2 / 417.4 / 408.2 / 408.9 / 406.2 ms で、健全時 8 回の最大は 445.2 ms）。**採った判断**: 判定予算を `cmd_vel_stale_ms + キープアライブ 2 周期 = 500ms` とした。1 周期目は上記の原理的な量子化、2 周期目は計測系の裕度（`t_last_cmd` はテスト側が publish した時刻であって `esp32_bridge` が受信した時刻ではなく、DDS 配送・送信コアレッシング（`D-1`）・WS 往復・モック側の記録が上乗せされる。当初 1 周期＝450ms で実装したところ最悪ケースの余裕が 8.4ms しか無く、境界フレークの実害があった）。**安全連鎖は別途機械的に守る**: `measured_ms < esp32_watchdog_ms`(600ms) を予算とは別のアサーションとして書き、さらに `_LATENCY_BUDGET_MS < esp32_watchdog_ms` をモジュールロード時に恒久チェックする（将来予算を広げすぎて `A7` が意図した順序を黙って飲み込むことを防ぐ）。**残る論点**: 設計書 §10 #12 の合格条件を「`cmd_vel_stale_ms` ＋ 送信周期 1 周期」と読み替えて明文化するか、`cmd_vel_stale_ms` を 350ms へ下げて予算合計を 400ms に収めるか、いずれ決める必要がある（後者なら `A7` の余裕も増える）。**このテストは変異テストで有効性を確認済み**——`keepalive_core.py` の stale 判定行を削除すると赤になる | 未決（[safety](DetailedDesign-safety.md) §10 #12。判定予算は暫定的に 2 周期で運用中） |
| **N-27** | **起動時の一過性の CPU ストールが `LIMITER_DEAD`（CRITICAL）を発火させ、`ESTOP` をラッチする。**2026-08-31 の実機 bringup（`stage:=1` / `lidar_source:=network`）で発生。起動 18 秒後に PC 側が約 260 ms 停止し、`obstacle_limiter` の 20 Hz 出力が `limiter_dead_ms`(250ms) を超えて途切れ、`fault.critical` → `C-06a` で `ESTOP` に落ちた（フォルト自体は 100 ms 後にクリアされたが、`ESTOP` はラッチなので自動復帰しない）。**PC 側のストールである根拠**: 同じ 1 秒窓で `esp32_bridge` も `feedback_gap_max_ms=259.9` を記録しており、リミッタ固有でも ESP32 固有でもない。18 秒時点なので `startup_grace_sec`(3 秒) の外。**発生源の推定**: Nav2 のライフサイクル起動と `dr_spaam_ros` のモデルロードが同時に走る時間帯と一致する。**対処の候補**（未決）: (a) 段階 1 の bringup で Nav2 / DR-SPAAM を起動しない、(b) `limiter_dead_ms` を上げる（安全側が弱まる）、(c) CRITICAL 判定に連続 N 窓のデバウンスを入れる、(d) `startup_grace_sec` を起動完了まで伸ばす（その間の本物のリミッタ死を見逃す）。**(b) と (d) は安全性を削るので最後の手段。**なお復帰は `/system/trigger` に `ui.estop.release` を投げれば `C-09`（ガード `no_critical_fault`）で `IDLE` へ戻る（実機で確認済み） **解決（2026-08-31。対処 (a) を採用）**: `bringup.launch.py` で Nav2 / SLAM / AMCL を段階 3 以上、leg_detection（DR-SPAAM）/ `person_tracker_bridge` を段階 4 以上に限定した。`required_nodes` は `[esp32_bridge, lidar_filter]` だけなので `evt.link_ok` には影響しない。**実機で再検証**: 起動〜90 秒で状態変化 0 回・フォルト 0 件（対処前は 18 秒で `ESTOP` ラッチ）。起動直後の `ESP32_DISCONNECTED` 一過性フォルトも消えた（起動が軽くなり ESP32 がタイムアウト前に繋がるようになった副次効果）。**安全側のしきい値は 1 つも緩めていない** | 解決（2026-08-31。[safety](DetailedDesign-safety.md) §5 / `th_state/config/transitions.yaml` `C-06a`・`C-09`） |
| **N-28** | **`registry.yaml` が `jog_gate` を `v_max` / `v_jog_panel` / `w_max` / `brake_accel_mps2` の `consumers` に挙げているが、`jog_gate` はこれらを一切使わない**（[wp2](DetailedDesign-wp2.md) `WP-SAFE-04` §3.3 が挙げるパラメータは `state_stale_ms` だけ）。`WP-SAFE-04` で `jog_gate` を `REGISTRY_NODES` に加えた結果、生成される `jog_gate.yaml` は**ノードが宣言していないパラメータを 5 個運ぶ**。**起動には影響しない**ことを実測で確認済み（2026-09-01。`ros2 run th_safety jog_gate --params-file` で 12 秒生存。rclcpp は未宣言の override を単に無視する）が、①`consumers` が「誰がこの値を使うか」の台帳として不正確になる、②`jog_gate` が blocking placeholder（`v_jog_panel` は未測定）の consumer に数えられ、将来 A8 が段階を上げたときに `jog_gate` を理由に起動が止まりうる、の 2 点で負債である。**`consumers` から `jog_gate` を外すのが正しい**が、A8 の対象範囲が変わるので `WP-PARAM-*` の作業として切り出す | 未決（`registry.yaml` の 4 行） |
