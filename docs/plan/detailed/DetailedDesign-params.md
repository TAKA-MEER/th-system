...
| **PT-4** | `overrides.yaml` の各行に `set_at` / `set_by` / `reason` が必須。**出どころの無い数値を作らない**（P2）。**`SetParams.srv` にはこれらのフィールドが無い**ので、`json` の中に `{"values": {...}, "set_by": "...", "reason": "..."}` の形で入れる（`WP-PARAM-02` で確定。[names](DetailedDesign-names.md) §5.2） |
