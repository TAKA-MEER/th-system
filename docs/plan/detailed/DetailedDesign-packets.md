...
| `WP-UI-08` | S-30 / S-31 / S-40 / S-50 | `WP-MAINT-01`〜`03` | S-50 に人物追跡 ON/OFF と開発モードがある（**開発モードの中身は `WP-DEV-01` へ切り出した**） |
| **`WP-DEV-01`** | **開発モードの実体**（警告の無視 ＋ 選択ログ記録） | `WP-UI-01`, `WP-PARAM-01` | 下記 §10.1 |
...
### 10.1 `WP-DEV-01` — 開発モードの実体（**実施順では最優先**。2026-09-20 ユーザー決定）

**段階 7 のパケットだが、実施は最優先**（[ImplementationPlan.md](../ImplementationPlan.md) §6 第 0 群）。
理由は**これ自体が以降すべての実装と検証を速くする**ため。機器が揃っていなくても先へ進めるので、
実機・ラズパイ・ESP32 のどれかが無い状態でも画面と FSM を動かして確かめられる。

正本は [Spec-webui.md](../spec/Spec-webui.md) §5 と [Spec-safety.md](../spec/Spec-safety.md) §10。
...
| 作ること | 内容 |
| --- | --- |
| 1. 無視する警告を**選ぶ** | 一括 ON/OFF ではなく項目ごと。対象は §10 の「無視できるもの」だけ |
| 2. 機器未接続でも先へ進める | S-00 で必須 3 者が揃うまで S-01 へ進めない制約を、**選んだときだけ**外す |
| 3. 手動系の自動ブレーキを場内でも OFF にできる | [Spec-safety.md](../spec/Spec-safety.md) §10 の表のとおり |
| 4. 始業点検の総合 NG を無視できる | 同上 |
| 5. ログの選択記録 | 選んだログだけタイムスタンプ付きで記録 |
| 6. 見た目 | 開発モード中は**ヘッダの色を変え、常に「開発モード」と表示**（実装済み） |
| 7. **ROS 側からも有効にできる** | **検証を行う実装管理担当（AI エージェントを含む）が WebUI を開かずに使えるようにする**（[Spec-webui.md](../spec/Spec-webui.md) §5.1）。起動引数と実行中のパラメータの両方。**`?dev=1` と S-50 のトグルは現状のまま変えない** |
| 8. 状態の正本を ROS 側に置く | いまはブラウザの `localStorage` だけ。**それでは ROS 側の警告を無視できない。**`dev_mode` を ROS 側の状態とし、画面はそれを読み書きする |
| 9. 記録 | 開発モードの ON/OFF と、**どの警告を無視しているか**をログに残す |

**無効化できないもの（構造で保証する。設定で外せてはいけない）**

- 物理非常停止ボタン ／ ESP32 ウォッチドッグ（安全チェーンの層 1・2）
- UI 非常停止ボタン
- 自律系の障害物停止

**`safety_monitor` と `obstacle_limiter` に `dev_mode` を渡さない**ことでこれを構造的に保証する
（両ノードが値を知らなければ、実装ミスで無効化されることが起きない）。この配線は変えない。

**完了条件**: ①機器を 1 つも繋がずに S-01 まで進み、画面と FSM の操作ができる
②開発モード ON で物理・UI の非常停止と自律系の障害物停止が**依然として効く**ことを故障注入で示す
③`dev_mode` が `safety_monitor` / `obstacle_limiter` のパラメータに現れないことを試験で縛る
④**WebUI を一度も開かずに**、起動引数だけで①と同じことができる（実装管理担当の検証経路）

**`SD-1`（操作系は WebUI のみ）に対する意図的な例外である。**開発モードは運用手順に含まれない
開発用の機能なので認める。**走行の操作を CLI から行えるようにするものではない。**

**`O-d6`（誰が有効化できるか・誤有効化の対策）は据え置き**（ユーザー決定 2026-09-20）。
**開発段階では制限をかけず、`?dev=1` と S-50 のトグルもそのまま使う。**
**通常運用に入る直前に対策を決める** —— このパケットを閉じるときに `O-d6` を消してはいけない。
