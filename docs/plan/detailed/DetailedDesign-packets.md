...
——実装者は結局 13 ファイル全部を読むことになる。

---

## 1. 絶対に守る順序制約

**この 5 件は、破ると「起動しない中間状態」または「安全装置が無い状態」を作る。**
...
| **O-5** | **`WP-ESP32-01`（バイパス解除）は実機で走らせる全パケットより前** | 物理非常停止が効かないまま走らせない（`DEBT-1`） |
...
| **7** | 保守 | **1 日の流れが全部 WebUI で完結**（`DEBT-1` 達成） |
| **8** | ライン誘導・電子リード | — |
...
| `WP-ESP32-01` | バイパス解除 ＋ `ESTOP_HW` にファーム構成フラグ（`DEBT-1`） | — | `safety_monitor` がバイパス有効を重大フォルトとして検出する |
...
**`DEBT-2`〜`DEBT-4` が解除され、`WP-ESP32-01` によりバイパスが重大フォルトとして検出されること。**
...
**段階 7 の出口**: 1 日の流れが全部 WebUI で完結する（`DEBT-1` 操作系は WebUI のみ、が達成される）。
...
| WP | 名前 | 依存 | 備考 |
| --- | --- | --- | --- |
| **`WP-LINE-00`** | **カメラの選定と帯域検証** | — | **実装より前に行う。**CSI 推奨。LiDAR を回しながら `/scan` の受信間隔が悪化しないことを実測（[transit](DetailedDesign-transit.md) §6.3） |
...
```
段階0  MEAS-01 ─┬─► PARAM-01 ─► PARAM-02 ─┐
       NET-01 ──┴─► MEAS-04 ──────────────┤
       MSG-01 ──────────────────┬─────────┤
       STATE-01 ────────────────┴─► STATE-02 ─► STATE-03
段階1                                │
       UI-01 ◄──────────────────────┘─► UI-02
段階2  SAFE-02 ─┐   ESP32-01      CALIB-01
...
**段階 0〜7 で 48 パケット**（0:10 / 1:6 / 2:7 / 3:4 / 4:5 / 5:4 / 6:6 / 7:6）。
