...
| **`WP-UI-08a`** | **設定値の調整パネル（最小版・開発モード内）** | `WP-PARAM-02`, `WP-UI-01` | `status: given` の行を一覧・編集・保存できる／**`derived` と `measured` は編集させない**／**`IDLE` / `MANUAL` 以外では受理しない**／アサーション違反の値は部分適用せず拒否される／`overrides.yaml` に `set_at` / `set_by` / `reason` が残る。**`WP-UI-08`（段階 7・S-50 の正式版）の前倒し・最小範囲**（[params](DetailedDesign-params.md) §5.3） |
...
**段階 0〜7 で 49 パケット**（0:**11** / 1:**7** / 2:**6** / 3:4 / 4:5 / 5:4 / 6:6 / 7:6）。
