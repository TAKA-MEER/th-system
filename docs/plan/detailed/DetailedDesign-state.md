...
正本 `Spec-modes.md` §3.1.1 / §3.1.2 の表に **ID 列がある**（`SM-3.1.1-01` / `SM-3.1.2-034`）。
行番号を使うと、正本を 1 行編集した瞬間に全 `spec_ref` がずれる。

> **2026-08-16**: 正本への ID 列追加は**完了した**（[open](DetailedDesign-open.md) §3 A-1）。
> 正本側の規約は「**欠番にしても ID を再利用しない**」。`WP-STATE-01` の着手前提条件は解けた。
...
**正本への ID 列追加は完了済み**（[open](DetailedDesign-open.md) §3 A-1・2026-08-16）なので、
`WP-STATE-01` は `spec_ref` の実値を入れた状態で着手できる。
...
`TEACH_MANUAL` の `REC` も同じ（**`T-TEACH-03M`** に `REC` 発の自己ループを含める）。
...
**行数が一致しないのは、正本が「A／B」と併記している契機を詳細設計側で 1 行 1 事象に割ったから**である
（下の「1 正本 ID ← 複数行」の 6 組がその全部）。**割った以外の増減は無い。**
...
**`SM-3.1.2-` は共通の接頭辞なので、下表では番号だけを書く。**

| モード | 写像 |
| --- | --- |
| `INIT` | `T-INIT-01`→001 ／ `-02`→002 ／ `-03`→003 |
| `FOLLOW` | `T-FOLLOW-01`→**004** ／ `-02`→**004** ／ `-03`→005 ／ `-04`→006 ／ `-05`→007 ／ `-06`→008 ／ `-07`→009 |
| `MANUAL` | `T-MANUAL-01`→010 ／ `-02`→011 |
| `TEACH_*` | `T-TEACH-01`→012 ／ `-02`→013 ／ `-03`→014 ／ `-03M`→015 ／ `-04`→016 ／ `-05`→017 ／ `-05M`→018 ／ `-06`→019 |
...
#### 4.4.3 1 つの正本 ID に複数の詳細行が対応する 6 組（**これが行数差の全部**）

| 正本 ID | 正本の書き方 | 詳細設計で割った理由 |
| --- | --- | --- |
| `SM-3.1.2-004` | 「対象を選ぶ**／**1 人だけの状態が `auto_select_hold_s` 続く」 | `ui.select_target` と `evt.auto_selected` は**入口が違う**（UI とノード）。§8 の名前空間分離により同じ行に載せられない |
| `SM-3.1.2-038` | 「走行開始」→ `RUN` **または** `HOLD` | **遷移先が違う。**ガード（`leash_taut` / `leash_slack`）で分けるので 2 行必要 |
| `SM-3.1.2-043` | 「2 点指示の②を押す**／**平面認識が完了」 | `evt.two_point_done` と `evt.plane_done` は別の事象 |
| `SM-3.1.2-069` | 「`clear_timeout_ms` 超過**／**追跡が切れた**／**「中止」」 | `evt.clear_timeout` / `evt.target_lost` / `ui.abort` の 3 事象。**案内の有無が違う**（`ui.abort` は案内を出さない） |
| `SM-3.1.2-094` | 「`S1` → `S2` → `S3` → `S4` 各ステップ完了」 | **effect が全部違う**（`run_measurement` / `build_preview` / `apply_and_verify` / `commit_calib`）。1 行にすると表駆動の意味が無くなる |
| `SM-3.1.1-10` | 「終了」（`ESTOP` 中は不可／`CARRY` は物理ボタン解除後のみ） | **ガードが違う。**`C-08` は `can_finish`、`C-12` は `hw_released` |

**この 6 組以外に、1 対 1 でない対応を作ってはいけない。**
作りたくなったら、それは**正本の行が 2 つの事象を混ぜている**という指摘なので、
先に `Spec-modes.md` へ行を足す（`Spec-open.md` §7.1 手順 6）。

#### 4.4.4 検査

```python
# th_testing/test/test_transition_table.py
SPEC_FANOUT = {"SM-3.1.1-10": 2, "SM-3.1.2-004": 2, "SM-3.1.2-038": 2,
               "SM-3.1.2-043": 2, "SM-3.1.2-069": 3, "SM-3.1.2-094": 4}

def test_fanout_is_exactly_the_declared_six():
    """1 正本 ID に複数行が対応するのは §4.4.3 の 6 組だけ。
    宣言していない fan-out が出たら、正本側に行が足りない。"""
```

**この検査が §11 のテスト 2・2r に加わる 3 本目**である。
2（欠落）と 2r（過剰）だけだと、**「1 行を 2 行に割った」ことを検出できない。**
