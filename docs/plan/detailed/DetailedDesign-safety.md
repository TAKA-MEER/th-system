...
する方針に改めた。「上位が急な指令を出しても機体側で鈍らせる」という正本の
規定自体は維持しつつ、鈍らせる層を1つから2つに増やす。

PC 側の `rate_limit()`（`follow_core.py`）は**追従にしか効かない**ので、
これだけでは足りない点は旧版から変更なし。

| 層 | 実装 | 効く範囲 | ランプされる入力 |
| --- | --- | --- | --- |
| **ESP32 ファーム** | `TARGET_RAMP_ACCEL_MPS2`（目標速度側）＋`PID_OUT_RAMP_RATE`（PID出力/PWM側、2026-09-11 追加。ただし setpoint=0 の停止指令には効かず常に即時0、2026-09-14 修正） | **すべての速度源**（ジョグ・Nav2・挙動ノード） | `/cmd_vel` として届いた非ゼロ値のみ |
| **`obstacle_limiter`**（2026-09-11 追加） | `normal_accel_mps2` で `/cmd_vel_muxed` の**生値（linear_x）**をランプしてから、安全上限を**ランプ後**の値にクランプする。`normal_angular_accel_rps2`（angular_z 側）は 2026-09-11 に実機で撤去（後述） | 同上（`obstacle_limiter` はすべての速度源の最終集約点） | `/cmd_vel_muxed` の linear_x のみ。安全上限自体はランプしない |
| PC 側の挙動ノード | `rate_limit()` | その方式の中だけ。**滑らかさのためであって安全のためではない** |

| 項目 | 決定 |
| --- | --- |
| 権威 | **ファーム側と `obstacle_limiter` の両方。**両方とも `registry.yaml` の値を持ち、A6 と同じく突き合わせる |
| 前提 | **重心が高くなることが予想される**ので、加減速はゆっくり（`Spec-safety.md` §3）。ESP32 側は現行ハードで確定値（`PID_OUT_RAMP_RATE=300`）とし、ハード改修で重心が上がった際に再チューニングする |
| 制動距離との関係 | `brake_accel_mps2`（実測）はファーム側ランプを含んだ**実際の止まり方**である。ファーム側ランプを変えたら**測り直す**。`obstacle_limiter` 側の通常時ランプは障害物ブレーキの制動距離には影響しない（次項の非対称設計） |

> **`obstacle_limiter` は「通常時の加減速」と「安全上限」を明確に分けて扱う。**
> `/cmd_vel_muxed` の生値を先にランプし（`ramped = ramp_toward(prev, muxed_input,
> rate*dt)`）、障害物ブレーキ（`v_allow`）・画面/モード上限・死角キャップ・
> estop・fault_lock・stale はすべて**ランプ後の値に対して従来どおり即座に
> クランプする**（`out = clamp(ramped, ceiling)`、`ceiling` 自体はランプしない）。
> ランプの起点 `prev` は**クランプ前の値ではなくクランプ後の実出力値**で更新する
> ——クランプ前の値を使うと、障害物接近でいったん絞られた直後に障害物が去った
> 瞬間、出力が急にジャンプしてしまうため。
>
> 結果としてこの非対称性が安全性を保つ: **危険が無い通常の解放・速度変更は
> ゆっくり、障害物ブレーキ・estop・fault_lock・stale はすべて従来どおり即座に
> 効き、制動距離は伸びない。**旧版が懸念していた「停止指令まで鈍って制動距離が
> 伸びる」は、estop/fault_lock/stale 由来のゼロ出力（tier 0〜2、`muxed` を見ずに
...
スルーレート由来の下限（速度換算 `PID_OUT_RAMP_RATE/KFF≈1.07 m/s^2`）付近で
頭打ちになった。ユーザー判断で「これ以上はソフトではなくハードの構造的な
問題」として **`normal_accel_mps2=0.75` を確定値**とし、これ以上は緩めない
（ハード改修で重心が変われば再検討）。

角速度側（`normal_angular_accel_rps2`）は当初 `normal_accel_mps2` と同じ理由
（転倒抑制）で同じ値(0.75)を導入したが、**超信地旋回（前進速度ゼロでのその場
回転）の立ち上がりが `w_max=1.2 rad/s` に対し `1.2/0.75≈1.6` 秒と遅くなる副作用
が実機で発覚**した（2026-09-04 WS-9T で「その場旋回が遅い」問題を解消した
ばかりの操作性を再び悪化させていた）。超信地旋回は前進を伴わず重心が横に
投げ出される動きではないため、前進しながらの急旋回ほど転倒リスクが高くない
とユーザーが判断し、**角速度側のランプは撤去した**（`normal_angular_accel_rps2
=0.0`＝無制限）。前進側の急変は `normal_accel_mps2` で引き続き抑制する。

**【2026-09-14】ファーム層での契約違反の発覚と修正**: 上の表は ESP32
ファーム層の `PID_OUT_RAMP_RATE` を「`/cmd_vel` として届いた値そのもの」に
効くと書いているが、実装（`pid.h` の `compute()`）では setpoint=0（停止
指令）の分岐にも無条件でこのスルーレートを適用していた。`obstacle_limiter`
の STOP・safety_monitor の fault_lock・stale 由来のゼロ出力は ESP32 からは
通常の `wheel_cmd=0` として届くため、**ファーム層で障害物ブレーキが従来
どおり即座に効くという前提が崩れていた**（`obstacle_limiter` 側の
非対称設計自体は正しく実装されていたが、その下のファーム層で無効化されて
いた）。`brake_accel_mps2=1.18`（WP-MEAS-01）はこの分岐が即時0を返す前提の
実測値であり、実機でダンボールに軽く接触する事故（停止はしたが制動距離が
想定より伸びた）が発生して発覚した。

修正: `pid.h` の `setpoint==0.0f` 分岐を即時0に戻した。`PID_OUT_RAMP_RATE`
