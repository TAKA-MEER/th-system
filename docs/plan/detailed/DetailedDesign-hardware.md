...
| 項目 | 仕様 |
| --- | --- |
| **バッテリー**（**2026-08-18 確定**） | **LiFePO4 8S・公称 24V・6Ah。実測最大 29.29V**（＝ 3.66V/セル。満充電） |
| **分圧** | **100 kΩ ＋ 10 kΩ（1/11）。**計算値 2.66V に対し **2026-08-18 の実測は 2.669V**（満充電 29.29V 時）。ADC 上限 3.3V に対し余裕がある。**分圧後に 3.3V を超えない比を選ぶこと**（超えると ADC が壊れる） |
| **しきい値** | `battery_warn_v = 24.0`（3.0V/セル。残量が急落する点）／ `battery_critical_v = 22.4`（2.8V/セル。劣化域）。**LiFePO4 は放電カーブが平坦**（25.6V 付近で大半の容量を使う）ため、電圧から残量%を出さない |
...
| モーター・エンコーダ | CuGoV3 の既定 | 既存・変更しない。ただし**カウント方式は要検討**——現在は 4 ピンすべてソフトウェア割り込みで、0.5 m/s で毎秒 3.2 万回の ISR が WiFi を阻害する（[open](DetailedDesign-open.md) `N-9`） |
| IMU | I2C | 既存・変更しない |
| **状態 LED** | **GPIO14** | §3.2。**2026-08-18 に実機で空きを確認して確定。** |
...
| 2 | **分圧抵抗 100 kΩ ＋ 10 kΩ**（§3.3 バッテリー電圧） | 段階 2 まで | **確定済み**（2026-08-18。LiFePO4 8S・実測最大 29.29V から算出） | — |
