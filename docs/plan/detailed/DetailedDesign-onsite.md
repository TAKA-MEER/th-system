...
→ **`WP-MEAS-02`（「人が歩いている部屋で地図を作ってみる」）が段階 0 にある。**
実装ではなく測定の作業パケットで、この結果が `WP-ONSITE-04` の範囲を決める
（[wp0](DetailedDesign-wp0.md) `WP-MEAS-02`）。

### 3.7 前日の地図を当日そのまま使うか（`O-d5` / `CL-M-7`）

**`Spec-open.md` は「実証実験の会場条件が分からないと決められない」として未確定のまま残している。**
決められないのは**方針**であって、**実装の形**は決められる。決めないと
「当日ずれていたときに何もできない」か「毎日作り直すしかない」の二択になる。

#### 3.7.1 何が問題か

| | |
| --- | --- |
| 前提 | 前日に `PREP` で地図とピンを作り、当日はその地図で `PANEL_NAV` / `SUMMON` / `HOME_NAV` を回す |
| 崩れる条件 | **当日、物の配置が変わっている**（納品物が搬入された・パレットが動いた・仕切りが増えた） |
| 何が起きるか | ① 自己位置推定が合わない ② Nav2 の costmap が実際と食い違い、**通れる経路を通れないと判断する／通れない経路へ突っ込む** ③ **ピンの座標は地図座標なので、地図がずれるとピンもずれる** |

**③ が最も重い。**①②は走れば分かるが、③は**盤の前に着いたつもりで別の場所に着く。**

#### 3.7.2 決めること — 「作り直す／そのまま使う」の二択にしない

**3 段の使い分けにする。**どれを採るかは当日その場で試験員が選ぶ（事前に決め打たない）。

| 段 | いつ | やること | 実装 |
| --- | --- | --- | --- |
| **A. そのまま使う** | 配置がほぼ同じ | 既存の `venue` 地図を `LOCALIZING` で開く。ピンはそのまま | 既存（`map_session` の `VENUE` スロット） |
| **B. 書き足す** | 一部だけ変わった | `A` に加えて **`map_update` フラグを ON にして走る。**差分は保存できる | **`REPLAY` の `map_update` と同じ仕組みを `PANEL_NAV` 系にも効かせる**（§3.7.3） |
| **C. 作り直す** | 大きく変わった／自己位置が合わない | `PREP` をやり直す。**ピンは 2 点指示で取り直す** | 既存 |

**B を用意するのが今回の決定である。**A と C は既にある。

#### 3.7.3 B の実装（`map_update` を試験場内モードへ広げる）
...
> **座標を勝手に動かすと、盤の前に着いたつもりで別の場所に着く**——③ をむしろ悪化させる。

#### 3.7.4 A か C かを試験員が判断できるようにする

**「地図が合っているか」を人が目で見て分かる形で出す。**判定を自動化しない
（合っている／いないの閾値を決める根拠がまだ無い ＝ `O-d5` が未確定である理由そのもの）。

| 表示 | 内容 | 出どころ |
| --- | --- | --- |
| **スキャンの重ね描き** | 地図の上に**現在の `/scan_filtered` を重ねて描く。**ずれていれば見た目で分かる | S-21 の地図タブ。`MapCanvas` |
| 自己位置の確度 | slam_toolbox の推定確度 | `/map_session/status` |
| **待機場所ピンとの実測ずれ** | 待機場所に居るはずの位置で `/onsite/declare_home` を押すと `offset_m` / `offset_deg` が返る | 既存（§4.1） |

**§4.1 の「待機場所にいることの宣言」が、そのまま地図ずれの測定器になる。**
新しい仕組みを作らない。**`offset_m` が大きければ C（作り直し）を勧める案内を出す。**

#### 3.7.5 申し送り

| 項目 | 扱い |
| --- | --- |
| **どの段を既定にするか** | **`O-d5` のまま。**会場条件が分かるまで決めない。実装は 3 段すべてを持つので、決まったら既定値を変えるだけ |
| `offset_m` の「大きい」の閾値 | **(c)。**`home_declare_tolerance_m` を流用するか別値にするかは実測後（[params](DetailedDesign-params.md) §6.2） |
| 作業パケット | **`WP-ONSITE-04` に含める**（`PREP` の地図修正と同じ `map_session` を触るため）。B の UI は `WP-UI-07`（S-21） |
